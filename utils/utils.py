...
def generate_text_simple(model, idx, max_new_tokens, context_size): # idx dim = (batch, n_tokens)
    for _ in range(max_new_tokens):
        idx_cond = idx[:, -context_size:]
        with torch.no_grad():
            logits = model(idx_cond)
        logits = logits[:, -1, :] # Focuses only on the last time step, dim = (batch, vocab_size)
